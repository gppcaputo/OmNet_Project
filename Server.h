...
class QUEUEING_API Server : public cSimpleModule, public IServer
{
    private:
        ...
        SelectionStrategy *selectionStrategy_2 = nullptr;

        Job *jobServiced = nullptr;
        cMessage *endServiceMsg = nullptr;

        void sendJobEl(Job *job); //aggiunto
        void sendJobIn(Job *job); //aggiunto
        void sendJob(Job *job, int gateIndex);// aggiunto

...
void PassiveQueue::handleMessage(cMessage *msg)
{
    ...
    if (k < 0) {
        ...
        job->setQueueCount(job->getQueueCount() + 1);
            ...
            sendJob(job,k);  // send through without queueing
...
void PassiveQueue::sendJobEl(Job *job) //modificato
{
...
void PassiveQueue::sendJobIn(Job *job) //modificato
{
...
bool PassiveQueue::isServerListIdle() {
    auto parent = getParentModule();
    int numServers = parent->par("numServers");
    auto svs = parent->getModuleByPath("servers[numServers]");

    cModule *server_list = new cModule[numServers];


    for(int i = 0; i < svs->getVectorSize(); i++){
        cout << svs[i].getName() << endl;
    }
    return true;
}
